...
func (mgr *pxeManagerT) ipxeBootScript(w http.ResponseWriter, r *http.Request) {
	...
	kernel := fmt.Sprintf("kernel %s/images/vmlinuz coreos.first_boot=1 initrd=initrd.cpio.gz coreos.config.url=%s?uuid=${uuid}&serial=${serial} systemd.journald.max_level_console=debug debug ignore_loglevel log_buf_len=10M coreos.autologin=tty0\n", mgr.pxeURL(), mgr.ignitionURL())
