...
func (mgr *pxeManagerT) ipxeBootScript(w http.ResponseWriter, r *http.Request) {
	...
	kernel := fmt.Sprintf("kernel %s/images/vmlinuz coreos.first_boot=1 initrd=initrd.cpio.gz coreos.config.url=%s?uuid=${uuid}&serial=${serial} systemd.journald.max_level_console=debug verbose log_buf_len=10M coreos.autologin=yes ip=10.0.201.110::10.0.201.12:255.255.255.0::eth5:none\n", mgr.pxeURL(), mgr.ignitionURL())
