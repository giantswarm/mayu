...
func (mgr *pxeManagerT) ipxeBootScript(w http.ResponseWriter, r *http.Request) {
	...
	if mgr.coreosAutologin {
		...
		mgr.logger.Log("level", "info", "message", "adding flatcar.autologin to kernel args")
	}

	// for ignition we use only 1phase installation without mayu-infopusher
	kernel := fmt.Sprintf("kernel %s/images/vmlinuz flatcar.first_boot=1 initrd=initrd.cpio.gz ignition.config.url=%s?uuid=${uuid}&serial=${serial} systemd.journald.max_level_console=debug verbose log_buf_len=10M "+extraFlags+"\n", mgr.pxeURL(), mgr.ignitionURL())
