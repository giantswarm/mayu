...
RUN apk add --no-cache git ca-certificates dnsmasq cpio
...
COPY scripts/fetch-coreos-qemu-image /usr/lib/mayu/fetch-coreos-qemu-image
...
ENTRYPOINT ["/mayu"]
CMD ["--cluster-directory=/var/lib/mayu"]
